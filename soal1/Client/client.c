...
{
    chdir("/home/hilmyhnf/modul3/soal1/Client");
    int fd = *(int *) fdc;
    char message[gas] = {0};
...
void *cekoutput(void *fdc) 
{
    ...
    while (1) 
    {
        ...
        if (strcmp(message, "Filepath: ") == 0) {
            cek = true;
        } else if (strcmp(message, "Sending file ...\n") == 0) {
            kirim(fd);
            cek = false;
        } else if (strcmp(message, "Uploaded file already exist.\n") == 0) {
            cek = false;
        } else if (strcmp(message, "Recieving file ...\n") == 0) {
            cetak(fd);
        } 
        fflush(stdout);
    }
...
void kirim(int fd)
{
    ...
    else 
    {
        ...
        send(fd, "File does not exist.", cekisi, 0);
    }
}
...
void cetak(int fd)
{
    ...
    //masukkan ke client
    puts("File successfully sent to server.");
    //send to server
    send(fd, "File successfully sent.", cekisi, 0);
    fclose(sends);
}
